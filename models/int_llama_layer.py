...
class QuantLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            past_key, past_key_scale = past_key_value[0]
            past_value, past_value_scale = past_key_value[1]

            past_key = self.k_cache_quantizer.dequantize(past_key, past_key_scale)
            past_value = self.v_cache_quantizer.dequantize(past_value, past_value_scale)

            key_states = torch.cat([past_key, key_states], dim=2)
            value_states = torch.cat([past_value, value_states], dim=2)

        if use_cache:
            key_states_int, key_scale = self.k_cache_quantizer.quantize(key_states)
            value_states_int, value_scale = self.v_cache_quantizer.quantize(value_states)
            past_key_value = ((key_states_int, key_scale), (value_states_int, value_scale))
        else:
            past_key_value = None
