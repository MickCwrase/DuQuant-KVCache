...
class QuantMistralDecoderLayer(nn.Module):
    def __init__(self, 
                 config: MistralConfig,
                 ori_layer,
                 args):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = QuantMistralAttention(
            org_module=ori_layer.self_attn,
            config=config,
            args=args,
            )
        self.mlp = QuantMistralMLP(
        ...
        self.input_layernorm = DuMistralRMSNorm(ori_layer.input_layernorm,eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = DuMistralRMSNorm(ori_layer.post_attention_layernorm,eps=ori_layer.post_attention_layernorm.variance_epsilon)
    ...
    def forward(
        ...
        # model parallel
        mlp_device = self.mlp.gate_proj.weight.device
        hidden_states = self.mlp(hidden_states.to(mlp_device)).to(residual.device)
        hidden_states = residual + hidden_states
